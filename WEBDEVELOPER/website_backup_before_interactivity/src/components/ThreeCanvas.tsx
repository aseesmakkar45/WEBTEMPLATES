...
function GlassOrbs({ stateRef }: { stateRef: React.MutableRefObject<AnimState> }) {
  ...
  // Keep track of mount time for the activation/power-on sequence
  const entryTimeRef = useRef<number | null>(null);

  // Materials using useMemo for optimal performance
  ...
  const detailMat = useMemo(
    () =>
      new THREE.MeshPhysicalMaterial({
        color: new THREE.Color("#C5F135"), // Lime accent highlights
        roughness: 0.3,
        metalness: 0.8,
        transparent: true,
        opacity: 0.8,
  ...
  const glowEyeMat = useMemo(
    () =>
      new THREE.MeshBasicMaterial({
        color: new THREE.Color("#ff003c"), // Glowing Red Indicator
        transparent: true,
        opacity: 0.0, // animated in useFrame
      }),
    []
  );
  ...
  useFrame((state) => {
    ...
    if (entryTimeRef.current === null) {
      entryTimeRef.current = state.clock.getElapsedTime();
    }

    const t = state.clock.getElapsedTime();
    const elapsed = t - entryTimeRef.current;
    const scroll = stateRef.current.scrollProgress;

    // 1. Reveal Sequence: Cylinders scale up + red irises flare up
    const focusProgress = Math.min(1.0, elapsed / 2.0); // 0 -> 1 over 2 seconds
    const flareProgress = Math.min(1.0, Math.max(0.0, (elapsed - 0.5) / 1.5)); // starts at 0.5s, reaches 1 at 2.0s
    
    const eyeScaleZ = 0.1 + focusProgress * 0.9;
    const eyeScaleXY = 0.5 + focusProgress * 0.5;
    
    // Pulsing iris glow
    const glowIntensity = flareProgress * (0.8 + Math.sin(t * 5.0) * 0.2);
    glowEyeMat.opacity = glowIntensity;

    // 2. Scroll and Fade-out gating (fades past 0.18 scroll progress)
    let activeScale = 1.0;
    if (scroll > 0.18) activeScale = Math.max(0.0, 1.0 - (scroll - 0.18) / 0.12);

    // Floating breathing motion
    groupRef.current.position.y = Math.sin(t * 0.35) * 0.10;

    // Scale entire helmet group
    groupRef.current.scale.setScalar(activeScale);

    // 3. Eye focusing animations (lenses rotating, cylinders shifting)
    if (leftEyeRef.current && rightEyeRef.current) {
      leftEyeRef.current.scale.set(eyeScaleXY, eyeScaleXY, eyeScaleZ);
      rightEyeRef.current.scale.set(eyeScaleXY, eyeScaleXY, eyeScaleZ);
      leftEyeRef.current.rotation.z = -t * 0.4;
      rightEyeRef.current.rotation.z = t * 0.4;
    }

    // 4. Parallax rotation tracked by mouse movements
    groupRef.current.rotation.y = THREE.MathUtils.lerp(
      groupRef.current.rotation.y,
      stateRef.current.mouseX * 0.22,
      0.04
    );
    groupRef.current.rotation.x = THREE.MathUtils.lerp(
      groupRef.current.rotation.x,
      -stateRef.current.mouseY * 0.15,
      0.04
    );
  });
  ...
  return (
    <group ref={groupRef} position={[0, 0, 0]}>
      {/* 1. Main Faceted Mask Plate */}
      <mesh material={helmetMat} position={[0, 0.2, 0]} rotation={[0.1, 0, 0]}>
        <icosahedronGeometry args={[1.35, 1]} />
      </mesh>

      {/* Visor shield piece */}
      <mesh material={helmetMat} position={[0, 0.1, 0.4]} scale={[1.1, 0.4, 0.9]}>
        <dodecahedronGeometry args={[0.9, 0]} />
      </mesh>

      {/* 2. Side Cheek/Ear Plates */}
      <mesh material={detailMat} position={[-1.2, -0.2, -0.2]} rotation={[0.2, 0.4, -0.1]} scale={[0.5, 0.9, 0.4]}>
        <boxGeometry />
      </mesh>
      <mesh material={detailMat} position={[1.2, -0.2, -0.2]} rotation={[0.2, -0.4, 0.1]} scale={[0.5, 0.9, 0.4]}>
        <boxGeometry />
      ...
      {/* Jaw plate */}
      <mesh material={helmetMat} position={[0, -0.8, 0.2]} scale={[0.8, 0.35, 0.6]}>
        <octahedronGeometry args={[0.8, 0]} />
      </mesh>

      {/* Visor wireframe overlay for futuristic UI feel */}
      <mesh material={wireframeMat} position={[0, 0.1, 0.41]} scale={[1.12, 0.42, 0.92]}>
        <dodecahedronGeometry args={[0.9, 0]} />
      </mesh>

      {/* 3. Left Mechanical Eye Assembly */}
      <group ref={leftEyeRef} position={[-0.52, 0.1, 0.9]} rotation={[0, 0.15, 0]}>
        <mesh material={lensMat} rotation={[Math.PI / 2, 0, 0]}>
          <cylinderGeometry args={[0.26, 0.26, 0.4, 20]} />
        </mesh>
        <mesh material={detailMat} position={[0, 0, 0.15]}>
          <torusGeometry args={[0.22, 0.03, 8, 24]} />
        </mesh>
        <mesh material={detailMat} position={[0, 0, 0.05]}>
          <torusGeometry args={[0.25, 0.015, 6, 24]} />
        </mesh>
        <mesh material={glowEyeMat} position={[0, 0, 0.08]}>
          <sphereGeometry args={[0.11, 16, 16]} />
        </mesh>
      </group>

      {/* 4. Right Mechanical Eye Assembly */}
      <group ref={rightEyeRef} position={[0.52, 0.1, 0.9]} rotation={[0, -0.15, 0]}>
        <mesh material={lensMat} rotation={[Math.PI / 2, 0, 0]}>
          <cylinderGeometry args={[0.26, 0.26, 0.4, 20]} />
        </mesh>
        <mesh material={detailMat} position={[0, 0, 0.15]}>
